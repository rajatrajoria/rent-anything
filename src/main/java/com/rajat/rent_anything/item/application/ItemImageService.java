...
public class ItemImageService {
    ...
    public ItemImageService(ItemRepository itemRepository, ItemImageRepository itemImageRepository, ImageStorageService imageStorageService) {
    ...
    public List<ItemImageResponseDto> uploadImages(Long itemId, Long userId, List<MultipartFile> files) {
        ...
        for (MultipartFile file : files) {
            ...
            ItemImageEntity savedImage = itemImageRepository.save(image);
            uploadedImages.add(toResponseDto(savedImage));
    ...
    public List<ItemImageResponseDto> getItemImages(Long itemId) {
        return itemImageRepository.findByItemIdOrderByDisplayOrderAsc(itemId).stream().map(this::toResponseDto).toList();
    }
    ...
    public void deleteImage(Long imageId, Long userId) {
        ...
        if (deletedThumbnail) {
            List<ItemImageEntity> remainingImages = itemImageRepository.findByItemIdOrderByDisplayOrderAsc(item.getId());
            if (!remainingImages.isEmpty()) {
                ItemImageEntity newThumbnail = remainingImages.get(0);
                newThumbnail.setThumbnail(true);
                itemImageRepository.save(newThumbnail);
            }
        }
        log.info("Deleted image {} for item {}", imageId, item.getId());
    }
