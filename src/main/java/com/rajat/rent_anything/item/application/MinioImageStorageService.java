...
public class MinioImageStorageService implements ImageStorageService {
    ...
    public MinioImageStorageService(MinioClient minioClient, StorageProperties storageProperties) {
    ...
    public String upload(Long itemId, MultipartFile file) {
        try {
            ...
            minioClient.putObject(PutObjectArgs.builder().bucket(storageProperties.getBucketName()).object(imageKey).stream(file.getInputStream(), file.getSize(), -1).contentType(file.getContentType()).build());
    ...
    public void delete(String imageKey) {
        try {
            minioClient.removeObject(RemoveObjectArgs.builder().bucket(storageProperties.getBucketName()).object(imageKey).build());
    ...
    public String getPublicUrl(String imageKey) {
        return storageProperties.getEndpoint() + "/" + storageProperties.getBucketName() + "/" + imageKey;
    }
}